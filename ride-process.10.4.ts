...
io.on("connection", (socket) => {
  ...
  socket.on("registerDriver", ({ driverId }) => {
    ...
    drivers[driverId] = { driverId, socketId: socket.id, online: true }; // <-- online by default
    socket.join(driverId);

    // Notify all users (optional) that driver is online
  ...
  //     io.to(driverId).emit("rideRequested", ride);
  //     console.log(`📤 Ride ${ride.rideId} sent to driver ${driverId}`);
  //   });
  // });
  ...
  //   // If a driverLocation is provided, set it (for dummy/testing)
  //   if (driverLocation) {
  //     ride.driverLocation = {
  //       lat: driverLocation.lat,
  //       lng: driverLocation.lng,
  ...
  //   // Emit to user room and driver room
  //   if (ride.userId) io.to(ride.userId).emit(`ride_${ride.rideId}`, ride);
  //   io.to(driverId).emit(`ride_${ride.rideId}`, ride);
  //   console.log(`✅ Ride ${rideId} accepted by driver ${driverId}`);
  // });
